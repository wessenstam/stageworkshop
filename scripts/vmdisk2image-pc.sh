...
VM_UUID=$(acli -H ${PE_HOST} -o json vm.list \
...
VMDISK_NFS_PATH=$(acli -H ${PE_HOST} -o json vm.get ${VM_NAME} include_vmdisk_paths=true \
...
if (( $? > 0 )) || [[ -z "${VMDISK_NFS_PATH}" ]]; then
  ...
  echo "VMDISK_NFS_PATH: nfs://${PE_HOST}${VMDISK_NFS_PATH}"
fi
...
nuclei image.create name=${IMG} \
  ...
  source_uri=nfs://${PE_HOST}${VMDISK_NFS_PATH}
